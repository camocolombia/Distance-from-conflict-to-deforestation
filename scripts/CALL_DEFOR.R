...
for(i in 1:length(def)){
  cat(round(i/length(def$id)*100,4)," %"," | ",i,"\n")
  def$dist[[i]]<-min(distm(def[i,],conflict)/1000,na.rm=T)
  def$conf_id[[i]]<-which.min(distm(def[i,],conflict)/1000)
